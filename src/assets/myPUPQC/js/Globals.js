...
function addFileFromUrl(dropzoneInstance, fileUrl, fileName) {
    return new Promise((resolve, reject) => {
        fetch(fileUrl)
            ...
            .then(blob => {
                ...
                // Emit Dropzone events to display the file
                dropzoneInstance.emit("addedfile", file);
                dropzoneInstance.emit("thumbnail", file, fileUrl); // Set preview
                dropzoneInstance.emit("complete", file); // Mark as complete

                // Mark file as already uploaded (so Dropzone does not try to upload it again)
...
$(function() {
    $.ajax({
        url: "http://127.0.0.1:8000/api/helpdesk/getUserRole",
        method: "GET",
