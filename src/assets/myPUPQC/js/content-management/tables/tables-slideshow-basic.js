...
$(function () {
  ...
  if (dt_select_table.length) {
    dt_select = dt_select_table.DataTable({
      ajax: assetsPath + 'myPUPQC/json/slideshow-list.json',
      columns: [
        // 1) Checkbox Column
        {
          data: 'id',
  ...
  $(document).on('click', '#slideshowTable .item-edit', function () {
    const slideShowId = $(this).data('id');
    console.log('Editing slide show:', slideShowId);
    $('#editImageUploadModal').modal('show');
  });

  // Single Delete => SweetAlert, then remove from DataTable
  $(document).on('click', '#slideshowTable .item-delete', function () {
    const slideShowId = $(this).data('id');
    Swal.fire({
      ...
      if (result.isConfirmed) {
        performSingleDelete(slideShowId);
      }
    });
  });
