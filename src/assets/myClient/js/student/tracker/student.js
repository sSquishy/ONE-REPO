$(document).ready(function() {
                ...
                response.forEach(chika => {
                    chikaContainerEl.append(`
                        <div class="col-12 col-md-6 col-lg-4 chikaCard" style="flex: 0 0 250px; height: 240px;">
                            <a href="#" data-bs-toggle="modal" data-bs-target="#anoangChikaModal">
                            <div class="card shadow-none border h-100 chikaCard" style="height: 150px; width: 300px; border-radius: 10px; overflow: hidden;">
                                ...
                                <div class="card-body">
                                <h5 class="card-title" style="font-size: small;">${chika.chikaTitle}</h5>
                                <p class="card-text"
                ...
                chikaContainerEl.append(`
                    <div class="col-12 col-md-6 col-lg-4 chikaCard" style="flex: 0 0 250px; height: 240px;">
                        ...
                        <div class="card shadow-none border h-100 chikaCard" style="height: 150px; width: 300px; border-radius: 10px; overflow: hidden;">
                            ...
                            <div class="card-body">
                            <h5 class="card-title" style="font-size: small;">Wala pa tayong chika ngayon.</h5>
                            <p class="card-text"
                                style="font-size: small; margin: 0; flex-grow: 1; overflow: hidden; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;">
    ...
    $(document).on('click', '.chikaCard', function() {
        ...
        const description = $(this).find('.card-text').text().trim();
        const image = $(this).find('.chikaImage').attr('src');

        $('#chikaCardTitle').html(title);
        $('#chikaCardText').html(description);
    ...
    function loadPosts() {
        ...
        $.ajax({
            ...
            beforeSend: function () {
                // Show the skeleton loader (e.g., placeholders)
                loadingTrigger.show();
            },
            success: function (response) {
                if (response.posts.length > 0) {
                    response.posts.forEach(post => {
                        ...
                        const postCard = `
                            <div class="card border rounded-3 p-3 mb-3 shadow-sm" style="background: #fff; width: 730px;">
                                <div class="card-header bg-transparent border-bottom-0 p-2 d-flex align-items-center" style="width: 700px;">
                                ...
                                <div class="card-body p-2" style="width: 700px;">
                                    <p class="mb-0 text-dark"
                                       style="font-size: 14px; line-height: 1.5; white-space: pre-wrap; word-wrap: break-word;">${contentHtml}</p>
                                </div>

                                ${ post.postImage.length > 0 ? `
                                  <div style="position: relative; width: 700px; height: 350px; overflow: hidden;">
                                    <!-- BLURRED BACKGROUND -->
                                    <div style="
                                      position: absolute;
                                      top: 0;
                                    ...
                                    <!-- ACTUAL IMAGE -->
                                    <a href="#" class="postImage" data-bs-toggle="modal" data-bs-target="#imageModal"
                ...
                // If there are no more posts, unobserve and hide the skeleton loader.
                if (!response.hasMore) {
