$(document).ready(function() {
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.posts.length > 0) {
          // Hide no pending message if visible
          $('#noPendingPosts').addClass('d-none');
          ...
          // Show no pending message only on first page
          if (currentPage === 1) {
            $('#noPendingPosts').removeClass('d-none');
          }
  ...
  function searchPendingPost() {
      ...
      feedContainer.empty();
      currentPage = 1;
      // Hide no pending message during new search
      $('#noPendingPosts').addClass('d-none');
      loadPosts();
  }
  ...
  function checkPendingPosts() {
      if (feedContainer.children('.card').length === 0) {
        // Show no pending message when no cards left
        $('#noPendingPosts').removeClass('d-none');
      }
  }
