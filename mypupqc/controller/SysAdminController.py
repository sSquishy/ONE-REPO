...
@transaction.atomic
def activateUploadedAccount(request, pendingUser):
    if request.method == 'POST':
        # Helper method to call appropriate addition of student or alumni
        def addStudentOrAlumni(isAlumni, user_number, birthYear, birthMonth, birthDay, mobileNo, emailAddress, webMail, programID, firstname, middlename, lastname, suffix):
            if isAlumni:
                addAlumni(
                    request, isFromBulkUpload=1,
                    isAlumni=isAlumni,
...
def bulkActivateUploadedAccount(request):
    if request.method == 'POST':
        ...
        for userID in userIDs:
            pendingUser = PendingAccount.objects.get(userID=userID)
            pendingUser.delete()
            threading.Thread(target=activateUploadedAccount, kwargs={"request": request, "pendingUser": pendingUser}).start()
...
def editBulkUploadAccount(request):
    if request.method == 'POST':
        ...
        user.suffix = request.POST.get('suffixEdit')
...
def initAdmin(request, email, accessString):
    if request.method == 'GET':
        ...
        try:
            ...
            return JsonResponse({'status': 'Failed to create admin credentials'})
...
def editFaculty(request):
    if request.method == 'POST':
        ...
        if checkExistingPermission('Admin', ifacultyID=faculty.credID, any=True):
            ...
            addPermissionList('Admin', ifacultyID=faculty.credID, isActive=is_admin)
        ...
        if checkExistingPermission('Moderator', ifacultyID=faculty.credID, any=True):
            ...
            addPermissionList('Moderator', ifacultyID=faculty.credID, isActive=is_moderator)
        ...
        if is_acad_head:
            if checkExistingPermission('Academic Head', ifacultyID=faculty.credID, any=True):
                ...
                addPermissionList('Academic Head', ifacultyID=faculty.credID, isActive=is_acad_head)
...
def editFacultyRoles(request):
    ...
    if checkExistingPermission('Admin', ifacultyID=faculty.credID, any=True):
        updatePermissionList('Admin', ifacultyID=faculty.credID, isActive=is_admin)
        print(f"Admin permission exists. is_admin: {is_admin}")
    elif is_admin:
        addPermissionList('Admin', ifacultyID=faculty.credID, isActive=is_admin)
...
def addAlumni(request, **kwargs):
    if request.method == 'POST':
        ...
        suffix = request.POST.get('suffixAdd') or kwargs.get('suffix', None)
        dateOfBirth = buildBirthday(
            request.POST.get('birthYearAdd') or kwargs.get('birthYear', None),
            request.POST.get('birthMonthAdd') or kwargs.get('birthMonth', None),
            request.POST.get('birthDayAdd') or kwargs.get('birthDay', None)
        )
        programID=request.POST.get('programAdd') or kwargs.get('programID', None)   
        mobileNo = request.POST.get('mobileNoAdd') or kwargs.get('mobileNo', None)
        emailAddress = request.POST.get('emailAddressAdd') or kwargs.get('emailAddress', None)
        webMail = request.POST.get('webmailAdd') or kwargs.get('webMail', None)
...
def editPersonnel(response):
    if response.method == 'POST':
        ...
        if checkExistingPermission('Admin', ifacultyID=personnel.credID, any=True):
            ...
            addPermissionList('Admin', ifacultyID=personnel.credID, isActive=is_admin)
        ...
        if checkExistingPermission('Academic Head', ifacultyID=personnel.credID, any=True):
            ...
            addPermissionList('Academic Head', ifacultyID=personnel.credID, isActive=is_acad_head)
...
def editPersonnelRoles(request):
    ...
    if checkExistingPermission('Admin', ipersonnelID=personnel.credID, any=True):
        ...
        addPermissionList('Admin', ipersonnelID=personnel.credID, isActive=is_admin)
    ...
    if checkExistingPermission('Academic Staff', ipersonnelID=personnel.credID, any=True):
        ...
        addPermissionList('Academic Staff', ipersonnelID=personnel.credID, isActive=is_acad_staff)
    ...
    if is_acad_head:
        if checkExistingPermission('Academic Head', ipersonnelID=personnel.credID, any=True):
            ...
            addPermissionList('Academic Head', ipersonnelID=personnel.credID, isActive=is_acad_head)
