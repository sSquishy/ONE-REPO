...
$(function () {
  ...
  var dt_alumni_table = $('.datatables-alumni'),
    ...
    statusObj = {
      ...
      3: { title: 'Pendng', class: 'bg-label-primary' }
    };
  ...
  if (dt_alumni_table.length) {
    var dt_alumni = dt_alumni_table.DataTable({
      ajax: assetsPath + 'myPUPQC/json/user-alumni-list.json', // JSON file to add data
      columns: [
            ...
            return `
              <div class="d-inline-block text-nowrap">
                ...
                <div class="dropdown-menu dropdown-menu-end m-0">
                  <a href="javascript:;" class="dropdown-item reset-password" data-id="${full.alumni_id}">
                    <i class="mdi mdi-lock-reset me-2"></i><span style="font-size: smaller;">Reset Password</span>
                  </a>
                  <a href="javascript:;" class="dropdown-item edit-alumni" data-id="${full.alumni_id}" data-bs-toggle="modal" data-bs-target="#editAlumniModal">
                    <i class="mdi mdi-pencil-outline me-2"></i><span style="font-size: smaller;">Edit</span>
                  </a>
                  <a href="javascript:;" class="dropdown-item delete-record" data-id="${full.alumni_id}">
      ...
      initComplete: function () {
        ...
        this.api()
          ...
          .every(function () {
            var column = this;
            var select = $(
              '<select id="user_alumniYear" class="form-select text-capitalize"><option value=""> Select Year </option></select>'
            )
              .appendTo('.user_alumniYear') // Ensure this container exists in your HTML
              .on('change', function () {
                var val = $(this).val();
        ...
        this.api()
          ...
          .every(function () {
            var column = this;
            var select = $(
              '<select id="AlumniProgram" class="form-select text-capitalize"><option value=""> Select Program </option></select>'
            )
              .appendTo('.user_alumniProgram')
              .on('change', function () {
...
$(document).on('click', '#alumni_resetPassword', function () {
  const selectedRows = dt_alumni.rows({ selected: true });
...
$(document).on('click', '#alumni_editSelected', function() {
  const selectedRow = dt_alumni.row({ selected: true });
  if (selectedRow.any()) {
    const rowData = selectedRow.data();
    $('#editAlumniModal').data('studentId', rowData.id).modal('show');
    console.log('Editing alumni:', rowData.id);
  }
});
...
function togglealumni_actionBar() {
  ...
  $('#alumni_actionBar').toggle(count > 0);
  $('#alumni_selectedCount').text(`${count} Selected`);
  $('#alumni_editSelected').toggle(count === 1).prop('disabled', count !== 1);
}

dt_alumni.on('select deselect', togglealumni_actionBar);
    ...
    $(document).on('click', '.reset-password', function () {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          console.log('Password reset for alumni ID ' + alumniId);
          Swal.fire({
    ...
    $(document).on('click', '.delete-record', function () {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          Swal.fire({
            title: 'Deleted!',
            text: "The alumni record has been deleted.",
            icon: 'success',
            customClass: {
              confirmButton: 'btn btn-success'
            },
